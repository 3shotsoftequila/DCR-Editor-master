...
function canConnect(source, target) {
  ...
  if (canConnectLink(source, target)) {
    return { type: 'dcr:Link' };
  }
  return false;
}

function canConnectLink(source, target) {
  return is(source, 'dcr:Object') && is(target, 'dcr:Object');
}
...
function canDrop(element, target) {
  ...
  return is(element, 'dcr:BoardElement') && is(target, 'dcr:DcrBoard');
}
...
function canResize(shape, newBounds) {
  ...
  //if (isAny(shape, [ 'dcr:Object', 'dcr:TextBox' ])) {
